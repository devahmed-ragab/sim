...
def start_button_fun():
    global counter
    start_button.configure(text="calculating")
    start_button.config(state="disabled")
    stop_action.config(state="enabled")
    if total_wattage_input.get() != "":
        gui_total_wattage = int(total_wattage_input.get())
...
total_wattage_input = tk.StringVar()
enterWidget = tk.Entry(main_frame, width=12, textvariable=total_wattage_input)
...
# ------------------------------------------------------------------------------------
start_button = ttk.Button(main_frame, text="Start", command=start_button_fun)
